...
summary: 'Puede utilizar el método GetIpmiInfo para mostrar un informe detallado de los sensores (objetos) de los ventiladores de los nodos, las temperaturas de entrada y salida y las fuentes de alimentación que supervisa el sistema.' 
---
= GetIpmiInfo
:allow-uri-read: 
:icons: font
...
[role="lead"]
Puedes usar el `GetIpmiInfo` Método para mostrar un informe detallado de los sensores (objetos) de los ventiladores de los nodos, las temperaturas de entrada y salida de aire y las fuentes de alimentación que son monitoreadas por el sistema.
...
Debido a la longitud de la respuesta devuelta por este método de la API, se han eliminado intencionadamente partes de la misma de este documento.  Se incluyen las partes de la información del hardware que el sistema monitoriza para garantizar que el nodo funcione con un rendimiento óptimo.
